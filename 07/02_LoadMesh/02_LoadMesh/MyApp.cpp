...
bool CMyApp::Init()
{
	...
	m_mesh->initBuffers();
	m_mesh1 = ObjParser::parse("Suzanne.obj");
	m_mesh1->initBuffers();
...
void CMyApp::Update()
{
	m_matView = glm::lookAt(glm::vec3(10, 10, 10),		// eye
...
void CMyApp::DrawMesh(glm::mat4 transformation, Mesh *mesh, GLuint texture)
{
	glUseProgram( m_programID );
...
void CMyApp::Render()
{
	...
	glm::mat4 trans1;
	auto angle = SDL_GetTicks() / 1000.0;
	auto angle1 = SDL_GetTicks() / 200.0;
	for (int i = 0; i < 6; ++i)
	{
		trans1 = glm::mat4(1.0f)
			* glm::rotate<float>(angle, glm::vec3(0, 1, 0))
			* glm::translate<float>(glm::vec3(10 * sin(2 * M_PI * i / 6), 2, 10 * cos(2 * M_PI * i / 6)))
			* glm::rotate<float>(-angle1, glm::vec3(0, 1, 0)); // 2nd transformation applied
		DrawMesh(trans1, m_mesh1, m_modelTextureID); // Drawing the loaded mesh
	}
}
