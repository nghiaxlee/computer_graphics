...
void CMyApp::Render()
{
	...
	// light
	float angle = SDL_GetTicks() / 500.0;
	glm::vec3 light_pos = glm::vec3(5 * cos(angle), 2, 5 * sin(angle));
	static auto loc = glGetUniformLocation(m_programID, "light_pos");
	glUniform3f(loc, light_pos.x, light_pos.y, light_pos.z);
	//eye
	static auto eye = glGetUniformLocation(m_programID, "eye_pos");
	glUniform3f(eye, m_eye.x, m_eye.y, m_eye.z);
	
	// PLAYAROUND
	float angle1 = SDL_GetTicks() / 1000.0;
	glm::vec3 color = glm::vec3(cos(angle1), sin(angle1), (cos(angle1) + sin(angle1)) / 2);
	static auto col = glGetUniformLocation(m_programID, "color");
