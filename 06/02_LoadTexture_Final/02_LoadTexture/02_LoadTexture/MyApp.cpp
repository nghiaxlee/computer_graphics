...
GLuint CMyApp::GenTexture()
{
    ...
    for (int i=0; i < 256; ++i)
        for (int j=0; j < 256; ++j)
        {
			int row = floor(i / cell_height);
			int col = floor(j / cell_width);

			if ((row + col) % 2)
			{
				tex[i][j][0] = 0; // Red channel
				tex[i][j][1] = 0; // Green channel
				tex[i][j][2] = 0; // Blue channel
			}
			else
			{
				tex[i][j][0] = 255; // Red channel
				tex[i][j][1] = 255; // Green channel
				tex[i][j][2] = 255; // Blue channel
			}
...
void CMyApp::Render()
{
	...
	DrawTexturedPlane(m_textureID, glm::translate<float>(glm::vec3(1, 1, 0)));
	DrawTexturedPlane(m_waterTextureID, glm::translate<float>(glm::vec3(-1, -1, 0)));
}
	...
	glm::mat4 transformation)
{
	...
	glUniform1i(glGetUniformLocation(m_programID, "time"),
		SDL_GetTicks());
